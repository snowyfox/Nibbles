# Nibbles eyes firmware

This is the eye firmware, one sub-project of the Nibbles shark totem. See the
root `CLAUDE.md` for the whole system (eyes, WLED controller, base station) and
`docs/architecture.md` for how the parts connect. Paths below are relative to
`firmware/eyes/`.
...
cd firmware/eyes                          # or pass -C firmware/eyes from the repo root
idf.py build
...
- Plays **bumps** aimed at the eyes (`nl_bump_t`, target bit `NL_TARGET_EYES`)
  ...
  - preset: shows preset number `arg` (1-based, so the same number means the
    same on WLED; index `arg - 1`) behind a swap blink, and blinks back to the
    eye's own preset on release (about 0.2 s to show: the lids take 0.14 s to
    shut). The leader changes presets only at `swap_now`, to whatever it
    should show (bump preset, else its own), and exports the preset on screen.
